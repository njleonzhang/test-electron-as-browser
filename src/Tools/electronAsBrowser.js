...
class BrowserLikeWindow extends EventEmitter {
  constructor(options) {
    ...
    this.controlView.setBounds(this.getControlBounds(false));
    this.controlView.setAutoResize({ width: true, height: true, horizontal: true, vertical: true });
    ...
    const channels = Object.entries({
      'control-ready': e => {
        log.debug('on control-ready', this.options.startPage);
        this.ipc = e;
  ...
  getControlBounds(tab = true) {
    ...
    return {
      ...
      height: this.options.controlHeight || (tab ? 90 : contentBounds.height)
    };
  }
   ...
   */
  newTab(url, appendTo) {
    log.debug('newTab in', url, appendTo)
    ...
    log.debug('current view', view.id, view.contextId);
